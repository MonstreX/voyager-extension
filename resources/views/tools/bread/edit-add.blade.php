                        ...
                        <div class="panel-body">
                            ...
                            @foreach($fieldOptions as $data)
                                ...
                                <div class="row row-dd">
                                    <div class="col-xs-3 bread-list-col">
                                        <h4 class="bread-list-field"><strong>{{ $data['field'] }}</strong></h4>
                                        <div class="bread-list-attr">
                                            <strong>{{ __('voyager::database.type') }}:</strong> <span>{{ $data['type'] }}</span>,
                                            <strong>{{ __('voyager::database.key') }}:</strong> <span>{{ $data['key'] }}</span>,
                                    ...
                                    <div class="col-xs-2 bread-list-col">
                                        @include('voyager-extension::tools.bread.action', ['data' => $data, 'action' => 'browse', 'color' => 'green'])
                                        @include('voyager-extension::tools.bread.action', ['data' => $data, 'action' => 'read', 'color' => 'green'])
                                        @include('voyager-extension::tools.bread.action', ['data' => $data, 'action' => 'edit', 'color' => 'green'])
                                        @include('voyager-extension::tools.bread.action', ['data' => $data, 'action' => 'add', 'color' => 'green'])
                            ...
                            @if(isset($dataTypeRelationships))
                                @foreach($dataTypeRelationships as $relationship)
                                    @include('voyager-extension::tools.bread.relationship-partial', $relationship)
                                @endforeach
                            @endif
...
@section('javascript')
    ...
    <script>
        ...
        $(function () {
            ...
            // Manage Ace Editor box
            $('.bread-list-ace-container').on('click', function(){
                const elAceWrapper = $(this).parent();
                if (!elAceWrapper.hasClass('opened')) {
                    elAceWrapper.addClass('opened');
                }
            });
